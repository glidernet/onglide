...
async function update_class(compClass, keys) {
    ...
    await db.query( escape`insert ignore into compstatus (class) values ( ${classid} )` );
...
async function update_pilots(class_url,classid,classname,keys) {
    ...
    await db.query( escape`DELETE FROM PILOTS WHERE class=${classid} AND registereddt < DATE_SUB(NOW(), INTERVAL 15 MINUTE)`);
...
async function process_day_task (day,classid,classname,latest_date_with_pilots,keys) {
    ...
    if( ! turnpoints._embedded['http://api.soaringspot.com/rel/points'] ) {
	console.log( "no turnpoints in task" );
	return;
    }
    
    const taskids = (await db.query( escape`
...
//        tplist[ tp.point_index ] = tp.name;
    ...
    await db.query( escape`UPDATE tasks SET flown='Y',task='A' WHERE taskid = ${taskid}` );
    await db.query( escape`DELETE FROM taskleg WHERE class=${classid} AND taskid != ${taskid} AND datecode = todcode(${date})` );
...
//    let [url] = (''+contest._links['http://api.soaringspot.com/rel/www']).match(/(http[^']*)/);
//    if( url ) {
//	await db.query( escape`UPDATE competition set siteroot=${url}` );
//    }

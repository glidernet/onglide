...
// Fix the turpoint types to what we know
...
module.exports.fetchSoaringSpot = function () {
    soaringSpot();
};
...
async function soaringSpot() {
    ...
    const contests = await sendSoaringSpotRequest( 'http://api.soaringspot.com/v1/', keys );
...
//        tplist[ tp.point_index ] = tp.name;
	...
	// so start point will have 0's
        await db.query( escape`INSERT INTO taskleg ( class, datecode, taskid, legno,
                    length, bearing, nlat, nlng, Hi, ntrigraph, nname, wleglength, type, direction, r1, a1, r2, a2, a12, quicktype )
               VALUES (
    ...
    await db.query( escape`call determinehcaplengths( ${classid} )`);

    // if it is today then set the briefing status properly
...
async function process_day_scores (day,classid,classname,latest_date_with_pilots,keys) {
    ...
    //    let task_details = await sendSoaringSpotRequest( day._links.self.href, keys );

    // Array of day objects
    day._embedded['http://api.soaringspot.com/rel/results'].forEach( async function (row) {
        ...
        //              hcapd = hcapd ? parseInt(hcapd) : 0; actuald = actuald ? parseInt(actuald.'0' x (7 - length actuald)) : 0;
        if( duration && row.scored_distance ) {

        let duration = (finish - start) / 3600;
        push( @scoredvals, (actuald / 1000 / duration) /3.6);
        push( @scoredvals, actuald );
        push( @scoredvals, row.scored_speed ); #(hcapd / 10000 / duration) * 3.6);
        push( @scoredvals, hcapd );
        }
        else {
        push( @scoredvals, 0 );
        push( @scoredvals, actuald );
        ...
        if( keys.actuals ) { // actuals on soaring spot (fai probably)
            //          console.log( "actuals pilot:".row._embedded['http://api.soaringspot.com/rel/contestant'].handicap + "->" + handicap + "->" + row.scored_speed );
...
//
async function update_contest(contest) {

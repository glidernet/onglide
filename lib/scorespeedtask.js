...
export default function scoreSpeedTask( taskObject, tracker, state, points ) {
    ...
    var task = taskObject.legs;

    // Always start at start for speed scored task
    tracker.scoredpoints = [ [ task[0].ll.dlong(), task[0].ll.dlat() ]];

    var t = 1;
    var p = points.length-2;

    let prevdist = Math.round(LatLong.distHaversine( points[p+1].ll, task[t].ll )*10)/10;
    var prevdistances = [];
    var prevpoint = [];
    var prevtime = [];
    var sectorpoints = 0;
	...
	var accumulated = 0;
	    ...
	    if( task[t].type == 'sector' ) {
                ...
                if( checkIsInTP(task[t], points[p]) >= 0 ) {
		    sectorpoints++;
		    insector = 1;
        ...
        // task
        var curdist = LatLong.distHaversine( points[p].ll, task[t].ll);
        var advancetp = 0;
...
//	console.log( tracker.compno + " ---> " + insector + ", " + sectorpoints + ", t " + t );
	...
	// Make sure we aren't further than the next leg is long
        var nextdist = Math.round(Math.min(LatLong.distVincenty( points[0].ll, task[t].ll),task[t].length)*10)/10;

...
export function PilotList({vc,selectedPilot,setSelectedPilot}) {
    ...
    const pilotList = _sortby(mutatedPilotList,['sortKey']).reverse()
          .map( (pilot) =>
		     ...
		     select={()=>{(selectedPilot&&selectedPilot.compno==pilot.compno)?setSelectedPilot(null):setSelectedPilot(pilot);}}/>
          );

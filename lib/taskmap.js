...
export function TaskMap( {vc,datecode,selectedPilot,mutatePilots,pilots} ) {
    ...
    const [ viewers, setViewers ] = useState({'c':0});
...
//	},
	...
	if(! updateMessage.keepalive ) {
		...
		viewers.c = updateMessage.listeners;
		console.log(updateMessage.listeners);
	    }
	}
    }
    ...
    return (<>
		...
		<span>{connectionStatus} {updateMessage ? updateMessage.t : null}</span><span style={{float:'right', textAlign:'right', width: '100px'}}>{viewers.c} <Icon type="group"/></span>
	    </div>
        </div>
	</>
    );

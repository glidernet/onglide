...
const scoreRefreshInterval = process.env.NEXT_SCORE_REFRESH_INTERVAL ? process.env.NEXT_SCORE_REFRESH_INTERVAL : 30000;
const trackRefreshInterval = process.env.NEXT_TRACK_REFRESH_INTERVAL ? process.env.NEXT_TRACK_REFRESH_INTERVAL : 2000;
...
export function usePilots (vc) {
    ...
    const { data, error } = useSWR( ['/api/scoreTask',vc], () => fetcher('/api/'+vc+'/scoreTask'),
...
export function usePilotsGeoJSON (vc) {
    const { data, error } = useSWR( ['/api/geoTracks',vc], () => fetcher('/api/'+vc+'/geoTracks'),

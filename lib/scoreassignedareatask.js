...
export default function scoreAssignedAreaTask( task, tracker, state, points ) {
    ...
    function log() { if( tracker.compno == '' ) { console.log(tracker.compno, ...arguments );} }
...
//                state.pointsByTime[points[p].t] = useP;
    ...
    dpoints.reverse().forEach( function(optimizedatstring) {
        ...
        if( leg == t && fakefinish ) { //&& tracker.stationary ) {
            log("generating final point as incomplete task",t,legs.length,previousPoint,point,minNextDistP);
			...
			distance = Math.min( LatLong.distHaversine(previousPoint,legs[t].ll), distance ); 
			distance = Math.max( distance, 0 );
            point = previousPoint.destPointRad( LatLong.bearing( previousPoint, legs[t].ll ), distance+(t==finishLeg?legs[t].maxR:0) );
        }

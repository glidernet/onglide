...
export default function scoreAssignedAreaTask( task, tracker, state, points ) {
    ...
    function log(x) { if( tracker.compno == 'OI' ) { console.log(x); }};
...
//            distance = Math.abs(
//				checkIsInTP(legs[t],{ll:previousPoint,geoJSON:thpoint([previousPoint.dlong(),previousPoint.dlat()])}) +
//				checkIsInTP(legs[t],{ll:point,geoJSON:thpoint([point.dlong(),point.dlat()])})
			//				);
            ...
            point = legs[t].ll.destPointRad( LatLong.bearing( legs[t].ll, previousPoint ), distance );
            //    log(point);
        }

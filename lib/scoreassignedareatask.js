...
//    function log(x) { if( tracker.compno == '' ) { console.log(x); }};
    function log(x,a,b,c,d) { if( tracker.compno == '' ) { console.log(tracker.compno, x, a||'', b||'', c||'', d||''); }};
    ...
    dpoints.reverse().forEach( function(optimizedatstring) {
        ...
        if( leg == t ) { //&& tracker.stationary ) {
            console.log(tracker.compno,"leg==t",t,legs.length,previousPoint,point,minNextDistP);

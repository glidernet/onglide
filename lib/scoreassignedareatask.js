...
export default function scoreAssignedAreaTask( task, tracker, state, points ) {
	...
	state.sectorpoints[0].push( {t:(tracker.utcstart), loc:(task[0].ll) } );
	state.lastProcessedTime = tracker.utcstart;
	state.pointsByTime[tracker.utcstart] = task[0].ll;
	t=1;
    }
	...
	var _wasinsector = checkIsInTP( task[t], points[p] );
	var _isinsector = _wasinsector;
	while( p > 0 &&  LatLong.distHaversine( points[p].ll, points[pprev].ll ) < 1 && (points[p].t-ptime) < 90 && (_wasinsector >= 0) == (_isinsector>=0) ) {
	    p--;
	    _isinsector = checkIsInTP( task[t], points[p] );
	}
	...
	// also confirm we have at least one point in the current sector or else we don't want to advance
        if( t != finishLeg ) {
	    if( (nextDistance = checkIsInTP( task[t+1],points[p] )) >= 0 && state.sectorpoints[t].length > 0 ) {
//		console.log( tracker.compno + "* in next sector, sector " + t + " has " + state.sectorpoints[t].length + " points in sector" );
	...
	//        if( checkIsInTP( task[t], points[p].ll ) >= 0 ) {
	if( _isinsector >= 0 ) {
...
//	    console.log( 'Point in finish sector' );
	...
	if( wasinsector ) {
		...
		state.sectorpoints[t-1].forEach( function(innerPoint) {
			...
			tempGraph.addLink( innerPoint.t, outerPoint.t, 1000-LatLong.distHaversine( task[t].ll, outerPoint.loc ));
		    }
		} );
	    } );
	}
	...
	// and circular sectors but not on wedges
	    ...
	    state.sectorpoints[t-1].forEach( function(previousPoint) {
		tempGraph.addLink( points[p].t, previousPoint.t, 1000-LatLong.distHaversine( task[t].ll, previousPoint.loc ));
	    } );
	}
...
//    console.log( tracker.compno + "* " + tracker.legspeeds );
	...
	var distance = LatLong.distVincenty(points[0].ll,task[leg].ll);

...
export function OgnFeed( {vc,datecode,mutatePilots,pilots,selectedPilot,mapRef,tz,viewport,setViewport} ) {
	...
	const times = useMemo( () => formatTimes(wsStatus.at, tz), [Math.round(wsStatus.at/60)]);
	
    // Render the map component
    return (<>
                <div style={{width: '100%', color:'grey', fontSize: '80%'}}>
                    <span>{connectionStatus} {wsStatus && wsStatus.at ? formatTimes(wsStatus.at,tz) : '' }</span>
                    <span style={{float:'right', textAlign:'right'}}><span className={'d-lg-inline d-none'}>In the last minute there were</span> {wsStatus.c} <Icon type="group"/> and {wsStatus.p} <Icon type="plane"/> tracked</span>
                </div>
            </>
           );
}
